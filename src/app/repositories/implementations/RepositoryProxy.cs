...
namespace backend.app.repositories.implementations
{
    ...
    public class RepositoryProxy<T> : DispatchProxy where T : class
    {
        ...
        public static T Create(T target, IRepositoryResiliencePolicy policy, IRepositoryAttributeResolver? resolver = null)
        {
            ...
            typed._interfaceToImpl = RepositoryProxyReflection.GetOrBuildMethodMap(typeof(T), target.GetType());
            return (T)proxy;
        }
        ...
        protected override object? Invoke(MethodInfo? targetMethod, object?[]? args)
        {
            ...
            if (behavior.NoRetry)
                return InvokeNoRetry(implMethod, args, targetMethod.ReturnType, behavior.HandleMissingEntity);

            return InvokeWithResilience(implMethod, args, targetMethod, behavior.HandleMissingEntity);
        }
        ...
        private object? InvokeNoRetry(MethodInfo implMethod, object?[] args, Type returnType, bool handleMissingEntity)
        {
        ...
        private object? InvokeWithResilience(MethodInfo implMethod, object?[] args, MethodInfo targetMethod, bool handleMissingEntity)
        {
            string operationName = BuildOperationName(targetMethod);
            Type returnType = targetMethod.ReturnType;

            if (returnType == typeof(Task))
                return ExecuteVoidThroughPolicy(implMethod, args, operationName, handleMissingEntity);

            if (IsTaskOfT(returnType))
            {
                Type resultType = returnType.GetGenericArguments()[0];
                return ExecuteTaskOfTThroughPolicy(implMethod, args, operationName, resultType, handleMissingEntity);
            }

            return InvokeTarget(implMethod, args);
        }

        private RepositoryMethodBehavior GetBehavior(MethodInfo interfaceMethod, MethodInfo implMethod)
        {
            if (_resolver != null)
            ...
            bool handleMissing = interfaceMethod.GetCustomAttribute<HandleMissingEntityAttribute>() != null;
        ...
        private object ExecuteVoidThroughPolicy(MethodInfo implMethod, object?[] args, string operationName, bool handleMissingEntity)
        {
            return _policy.ExecuteAsync(
                _ => handleMissingEntity
        ...
        private object? ExecuteTaskOfTThroughPolicy(MethodInfo implMethod, object?[] args, string operationName, Type resultType, bool handleMissingEntity)
        {
            MethodInfo methodInfo = RepositoryProxyReflection.GetOrResolveExecuteMethod(GetType(), resultType, handleMissingEntity);
            try
            {
        ...
        private async Task<TResult> ExecuteWithPolicy<TResult>(MethodInfo implMethod, object?[] args, string operationName)
        {
        ...
        private async Task<TResult> ExecuteWithPolicyAndHandleMissingEntity<TResult>(MethodInfo implMethod, object?[] args, string operationName)
        {
            return await _policy.ExecuteAsync(
        ...
        private async Task InvokeVoidAndHandleMissingEntityAsync(MethodInfo implMethod, object?[] args)
        {
            try
            {
        ...
        private async Task<TResult> InvokeAndHandleMissingEntityAsync<TResult>(MethodInfo implMethod, object?[] args)
        {
            try
            {
        ...
        private static object WrapTaskWithMissingEntityHandling(Task task, Type returnType)
        {
            ...
            Type resultType = returnType.GetGenericArguments()[0];
            MethodInfo? wrap = typeof(RepositoryProxy<T>).GetMethod(nameof(WrapTypedAsync), BindingFlags.NonPublic | BindingFlags.Static);
            if (wrap is null)
                throw new InvalidOperationException($"Could not find method {nameof(WrapTypedAsync)} on {nameof(RepositoryProxy<T>)}.");
        ...
        {
            try { return await ((Task<TResult>)task); }
            catch (Exception ex) when (IsMissingEntityOrConcurrency(ex)) { return default!; }
        }
    }
}
